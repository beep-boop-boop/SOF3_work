...
all' pred xs = getAll (foldMap (All . pred) xs) 
--any' pred xs = getAny (foldr (\x acc -> Any (pred x) <> acc) mempty xs)
any' pred xs = getAny (foldMap (Any . pred) xs) 
{-
...
compact fromB p xs = extract (foldMap (fromB . p) xs)
{-
...
unitMul (Succ x) = 
  oneN /*/ Succ x
  :=:
  Succ x /*/ oneN -- commutativity of multiplication
  :=:
  Succ x /+/ (Succ x /*/ Zero) --Mul.1
  :=:
  Succ x /+/ Zero -- Mul.0
  :=:
  Succ x -- Add.0
...
-}
sumOddSq :: Nat -> ProofLayout Nat
sumOddSq (Succ x) =
   sumOdd x /+/ subOne (twoN /*/ Succ x)
   :=:
   sqn x /+/ subOne ((twoN /*/ x) /+/ twoN) -- induction step
   :=:
   sqn x /+/ (twoN /*/ x) /+/ oneN
   :=:
   sqSucc x
...
sqSucc' x = 
  ...
  Succ x /*/ Succ x -- sqn.0
  :=:
  Succ x /+/ (Succ x /*/ x) -- Mul.1
  :=:
  Succ x /+/ (x /*/ Succ x) --commutativity of multiplication
  :=:
  Succ x /+/ (x /+/ (x /*/ x)) --Mul.1
  :=:
  Succ x /+/ x /+/ sqn x -- sqn.0
  :=:
  x /+/ oneN /+/ x /+/ sqn x -- definition of successor
  :=:
  sqn x /+/ (x /+/ x) /+/ oneN -- associativity of addition
  :=:
  sqn x /+/ (x /+/ (x /*/ oneN)) /+/ oneN -- unitMul
  :=:
  sqn x /+/ (x /*/ twoN) /+/ oneN -- Mul.1
  :=:
  sqn x /+/ (twoN /*/ x) /+/ oneN -- commutativity of multiplication
  :=:
  QED