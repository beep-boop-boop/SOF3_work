...
import Graphics.Win32 (bST_CHECKED, aNSI_CHARSET)
...
cakeBill quantity price = quantity * price
...
cakeBill' :: Int -> Int -> String
cakeBill' 1 price = "The cost of 1 cake at " ++ price ++ "p each is " ++ cakeBill quantity price ++ "." 
cakeBill' quantity price  = "The cost of " ++ quantity ++ " cakes at " ++ price ++ "p each is " ++ cakeBill quantity price ++ "."
...
implies_ False b = True
implies_ True b = b
implies_ a True = True 
implies_ a False = not a
...
implies'' :: Bool -> Bool -> Bool
implies'' _ True  = True -- using "don't care" patterns
...
matches (input, output) = input == output
...
-}
insert :: Ord a => a -> [a] -> [a] -- assumes list is ordered in ascending order
