...
-}
oneone, oneone', oneone'' :: [Int] -> [Int]
oneone ns = [ n+1 | n <- ns,  n `mod` 2 == 0 ]
oneone' = foldr (\n xs -> if n `mod` 2 == 0 then n : xs else xs) []
oneone'' = map (1 +) . filter even
...
-- the function takes an input function and continuously applies it to an initial input until the function doesnt change the input (finds a fixed point!)
{-
### Q3.6
...
-}
vm1_h' :: VM
vm1_h' = Coin `leadsto` (Choc `leadsto` (vm1_h' `branch` (Halt `leadsto` stop)))
...
depth tree = sum [1 | x <- show tree, x == '['] - 1 -- idk man

takeTree 0 _ = stop
takeTree d (Tree ts) = Tree (map take' ts) where
  take' (a, b) = (a, takeTree (d - 1) b)
...
countTree (Tree ts) = sum (map count' ts) where
  count' (_, b) = 1 + countTree b
...
mapTree f (Tree ts) = Tree (map apply ts) where
  apply (a, b) = (f a, mapTree f b)
